...
/**
 * @description 检测在对应方向上能否允许滑动，比如SlideHorizontal组件就只处理左右滑动事件，SlideVertical只处理上下滑动事件
...
export function canSlide(state) {
  ...
  if (state.needCheck) {
    // FIXME: 判断move x和y的距离是否大于判断值，因为距离太小无法判断滑动方向
...
export function slideTouchMove(
  ...
  // FIXME: 特别处理：竖直的slide组件，在第一页往下滑动时，向外发送事件
  // 用于首页顶部导航栏的刷新动画
...
export function slideTouchEnd(e, state, canNextCb = null, nextCb = null, notNextCb = null) {
  ...
  if (state.next) {
    ...
    if (!canNextCb) canNextCb = canNext
    if (canNextCb(state, isNext)) {
      // 换成pointer事件之后不能捕获了，需要让父组件重置自己的isDown，不然PC上move事件会一直触发
      // _stopPropagation(e)
 ...
 * @param emit
 */
export function slideReset(e, el, state, emits = null) {
  if (!checkEvent(e)) return
  ...
  emits?.('update:index', state.localIndex)
}
